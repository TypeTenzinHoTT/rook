...
export async function notify() {
        ...
        const { url } = await inquirer.prompt([
            {
                ...
                validate: (val) => (!!val && val.startsWith('http') ? true : 'Enter a valid URL')
            }
        ]);
        const { spinner, slowTimer } = startSpinnerWithSlowNotice(`Saving ${type} webhook...`);
        try {
            await saveNotificationIntegration(type, url);
    ...
    try {
        ...
        if (integrations?.length) {
            ...
            integrations.forEach((i) => console.log(`- ${i.type}: ${i.webhook_url}`));
        }
    }
    catch {
        // ignore
