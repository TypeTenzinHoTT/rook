...
export async function dungeon() {
    ...
    try {
        ...
        const questStreak = stats?.questStreak || 0;
        const questStreakBonus = stats?.questStreakBonus;
        const battles = stats?.prBattles || [];
        const battleLines = battles && battles.length
            ? `\n\n${chalk.blue('⚔️ PR Battles:')}\n${battles.map((b) => `- vs ${b.opponent} — ${b.status || 'pending'}`).join('\n')}`
            : '';
