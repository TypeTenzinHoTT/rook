...
function pickPlatform(opts: ShareOptions): string {
  if (opts.twitter) return 'twitter';
  if (opts.discord) return 'discord';
  if (opts.slack) return 'slack';
  return 'link';
}

export async function share(target: string, achievementId?: string, opts: ShareOptions = {}) {
  guardLogin();
