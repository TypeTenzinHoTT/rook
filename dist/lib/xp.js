...
export function getLevelTitle(level) {
    ...
    if (level >= 25)
        return 'Mythic Innovator';
    if (level >= 20)
        return 'Master Developer';
    if (level >= 15)
        return 'Senior Spellcaster';
    if (level >= 10)
        return 'Battle-Hardened Coder';
    if (level >= 5)
        return 'Apprentice Debugger';
    return 'Rookie Adventurer';
}
export function progressBar(current, total, width = 24) {
    if (total <= 0)
        return '[................] 0%';
...
export function calculateStreak(lastActive, currentStreak) {
